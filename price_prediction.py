...
min_price = st.slider('Minimum Price/SqFeet', min_value=0, max_value=int(data['price'].max()), step=1)
max_price = st.slider('Maximum Price/SqFeet', min_value=0, max_value=int(data['price'].max()), step=1)
...
if st.button("Predict"):
    ...
    try:
        with st.spinner('Predicting...'):
            ...
            total_price = price_per_sqft * total_sqft

        st.success('Done!')
        st.write(f"{model_choice} Total Price: {total_price}")
